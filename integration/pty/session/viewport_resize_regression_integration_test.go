...
func TestHostResizeCtrlLClearAfterExpandClearsPreservedContent(t *testing.T) {
	...
	waitForSessionCountSession(t, h.Clock(), h.Endpoint(), h.AccessToken(), h.AuthFile(), 2, 6*time.Second)
	host.Send("clear; for i in $(seq 1 40); do printf 'CLEAR-%02d-LEFT-1234567890-RIGHT-%02d-END\\n' \"$i\" \"$i\"; done\n")
	eventuallyWithClock(t, h.Clock(), 4*time.Second, 50*time.Millisecond, func() error {
		screen := host.Screen().String()
		if !strings.Contains(screen, "PROMPT>") {
			return fmt.Errorf("expected prompt after initial ps output, got:\n%s", screen)
		}
		if !strings.Contains(screen, "CLEAR-40-LEFT-1234567890-RIGHT-40-END") {
			return fmt.Errorf("expected initial deterministic output on screen, got:\n%s", screen)
		}
		row := host.Screen().Row(0)
	...
	eventuallyWithClock(t, h.Clock(), 2*time.Second, 50*time.Millisecond, func() error {
		...
		if !strings.Contains(screen, "CLEAR-40-LEFT-1234567890-RIGHT-40-END") {
			return fmt.Errorf("expected expand to restore deterministic output, got:\n%s", screen)
		}
		return nil
	})
	_ = host.DrainRaw()
	...
	eventuallyWithClock(t, h.Clock(), 2*time.Second, 50*time.Millisecond, func() error {
		...
		if strings.Contains(screen.String(), "CLEAR-40-LEFT-1234567890-RIGHT-40-END") {
			return fmt.Errorf("expected clear to remove prior deterministic content after resize restore, got:\n%s", screen.String())
		}
		return nil
	})
}

func emitDeterministicLargeOutputAndWaitForPromptNumberSession(t *testing.T, host *ptytest.PTYSession, nextPrompt int, timeout time.Duration) {
	t.Helper()
	emitDeterministicLargeOutputSession(t, host, timeout)
	waitForHostPromptNumber(t, host, nextPrompt, timeout)
}

func emitDeterministicLargeOutputSession(t *testing.T, host *ptytest.PTYSession, timeout time.Duration) {
	t.Helper()
	...
	eventuallyWithClock(t, host.Clock(), timeout, 50*time.Millisecond, func() error {
		if !host.Screen().Contains("ROW-120 0123456789012345678901234567890123456789") {
			return fmt.Errorf("expected deterministic large output, got:\n%s", host.Screen().String())
		}
		return nil
	})
}
...
func TestHostResizeTypingWhileShrunkAfterPsAuxKeepsPromptOnBottomRow(t *testing.T) {
	...
	emitDeterministicLargeOutputAndWaitForPromptNumberSession(t, host, 2, 4*time.Second)
	eventuallyWithClock(t, h.Clock(), 2*time.Second, 50*time.Millisecond, func() error {
		screen := host.Screen()
		if !screen.Contains("ROW-120 0123456789012345678901234567890123456789") || !screen.Contains("PROMPT-002>") {
			return fmt.Errorf("expected deterministic large output with prompt before shrink, got:\n%s", screen.String())
		}
		return nil
	})
	_ = host.DrainRaw()
...
func TestHostResizeWhileShrunkWithDecoratedPromptKeepsPromptVisible(t *testing.T) {
	...
	emitDeterministicLargeOutputSession(t, host, 4*time.Second)
	eventuallyWithClock(t, h.Clock(), 2*time.Second, 50*time.Millisecond, func() error {
		screen := host.Screen()
		if !screen.Contains("ROW-120 0123456789012345678901234567890123456789") || !screen.Contains("~{}") {
			return fmt.Errorf("expected deterministic large output with decorated prompt before shrink, got:\n%s", screen.String())
		}
		return nil
	})
...
func TestHostResizeImmediateTypingAfterShrinkKeepsPromptOnBottomRow(t *testing.T) {
	...
	eventuallyWithClock(t, h.Clock(), 2*time.Second, 50*time.Millisecond, func() error {
		screen := host.Screen()
		if !screen.Contains("ROW-120 0123456789012345678901234567890123456789") || !screen.Contains("PROMPT-002>") {
			return fmt.Errorf("expected deterministic large output with prompt before immediate shrink typing, got:\n%s", screen.String())
		}
		return nil
	})
...
func TestHostResizeLargeViewportClearAfterExpandMatchesControl(t *testing.T) {
	...
	emitDeterministicLargeOutputSession(t, host, 4*time.Second)
	emitDeterministicLargeOutputSession(t, control, 4*time.Second)
