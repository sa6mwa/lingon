...
func waitForSessionCountSession(t *testing.T, clk clock.Clock, endpoint, token, authPath string, want int, timeout time.Duration) {
	...
	for clk.Now().Before(deadline) {
		if authPath != "" {
			tlsDir := filepath.Join(filepath.Dir(authPath), "tls")
			if refreshed, err := relayclient.EnsureAccessTokenWithTLSDir(context.Background(), endpoint, authPath, tlsDir); err == nil && refreshed.AccessToken != "" {
