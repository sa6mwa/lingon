...
func TestWebUIControlFlow(t *testing.T) {
	...
	userDataDir := filepath.Join(configDir, "chromedp")
...
func TestWebUIAccountSeparation(t *testing.T) {
	...
	runLogin(userA.User.Username, "pass-a", userA.TOTPSecret, "web-session-a", "READY_A", "READY_B", filepath.Join(configDir, "chromedp-a"))
	runLogin(userB.User.Username, "pass-b", userB.TOTPSecret, "web-session-b", "READY_B", "READY_A", filepath.Join(configDir, "chromedp-b"))
	...
	runShare(shareA.Token, "READY_A", "READY_B", filepath.Join(configDir, "chromedp-share-a"))
	runShare(shareB.Token, "READY_B", "READY_A", filepath.Join(configDir, "chromedp-share-b"))
}
...
func TestWebUIShareTokenCookieReloadAndLogout(t *testing.T) {
	...
	ctx, cancel := newChromedpContext(t, filepath.Join(configDir, "chromedp-share-cookie"))
	defer cancel()
	if err := chromedp.Run(ctx, network.Enable()); err != nil {
		t.Fatalf("chromedp network enable: %v", err)
	}
...
func TestWebUIFullscreenSingleLayout(t *testing.T) {
	...
	ctx, cancel := newChromedpContext(t, filepath.Join(configDir, "chromedp-fullscreen"))
	defer cancel()
	if err := chromedp.Run(ctx, network.Enable()); err != nil {
		t.Fatalf("chromedp network enable: %v", err)
	}
...
func TestWebUITabOverflowAutoScrollAndFades(t *testing.T) {
	...
	ctx, cancel := newChromedpContext(t, filepath.Join(configDir, "chromedp-tab-overflow"))
	defer cancel()
	if err := chromedp.Run(ctx,
		network.Enable(),
...
func TestWebUIResizeDoesNotBypassReconnectBackoff(t *testing.T) {
	...
	ctx, cancel := newChromedpContext(t, filepath.Join(configDir, "chromedp-backoff-resize"))
	defer cancel()
	if err := chromedp.Run(ctx, network.Enable()); err != nil {
		t.Fatalf("chromedp network enable: %v", err)
	}
...
func TestWebUISwitchesToNewActiveSessionAfterNoHost(t *testing.T) {
	...
	ctx, cancel := newChromedpContext(t, filepath.Join(configDir, "chromedp-switch-active"))
	defer cancel()
	if err := chromedp.Run(ctx, network.Enable()); err != nil {
		t.Fatalf("chromedp network enable: %v", err)
	}
...
func TestWebUIManualRefreshButtonDiscoversSessions(t *testing.T) {
	...
	ctx, cancel := newChromedpContext(t, filepath.Join(configDir, "chromedp-manual-refresh"))
	defer cancel()
	if err := chromedp.Run(ctx, network.Enable()); err != nil {
		t.Fatalf("chromedp network enable: %v", err)
	}
...
func TestWebUIConnectAndReloadDoNotRearmWallInactivityWithoutTerminalInput(t *testing.T) {
	...
	ctx, cancel := newChromedpContext(t, filepath.Join(configDir, "chromedp-wall-idle"))
	defer cancel()
	if err := chromedp.Run(ctx, network.Enable()); err != nil {
		t.Fatalf("chromedp network enable: %v", err)
	}
