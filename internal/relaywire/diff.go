...
func DiffSnapshots(prev, next *protocolpb.Snapshot) (*protocolpb.Diff, bool) {
	...
	includeGraphemes := len(next.Graphemes) > 0 || len(prev.Graphemes) > 0
	for y := 0; y < rows; y++ {
		...
		if includeGraphemes {
			row.Graphemes = nextRowStrings(next.Graphemes, y, cols)
		}
...
func nextRowStrings(data []string, row, cols int) []string {
