...
// RegenerateAll removes existing local TLS assets and generates new ones.
func RegenerateAll(ctx context.Context, dir, hostname string, logger pslog.Logger) error {
	logger = ensureLogger(logger)
	...
	for _, name := range []string{caCertFilename, caKeyFilename, serverCertFilename, serverKeyFilename} {
	...
	return GenerateAll(ctx, dir, hostname, logger)
}
...
func generateCA(dir string, logger pslog.Logger) error {
	...
	logger.Info("tls.ca.generate.done", "cert", filepath.Join(dir, caCertFilename))
	return nil
}
...
func generateServerCert(dir, hostname string, logger pslog.Logger, allowMissingCA bool) error {
	...
	logger.Info("tls.server.generate.done", "cert", filepath.Join(dir, serverCertFilename))
	return nil
}
