...
func TestDiffSnapshotsClearsGraphemes(t *testing.T) {
	prev := snapshotForTest(2, 1, []rune{'a', 'b'})
	...
	if diff == nil || len(diff.DiffRows) != 1 {
		t.Fatalf("expected one diff row")
	}
	if len(diff.DiffRows[0].Graphemes) != 2 {
		t.Fatalf("expected grapheme row length 2, got %d", len(diff.DiffRows[0].Graphemes))
	}
	if diff.DiffRows[0].Graphemes[0] != "" {
		t.Fatalf("expected grapheme cleared")
	}
}
